<template>
  <AppLayout>
    <div class="container mx-auto py-10 px-2">
      <h1 class="mb-6 text-2xl font-bold text-gray-900 text-center">Job Applications</h1>
      ...
      <div v-if="jobApplications.length" class="space-y-6">
        <div
          v-for="application in jobApplications"
          :key="application.id"
          class="relative flex flex-col gap-2 rounded-xl border border-gray-200 bg-white p-4 shadow-sm hover:shadow-md transition dark:border-gray-700 dark:bg-gray-900"
        >
          ...
          <div class="text-xs text-gray-500 dark:text-gray-400">Applied: {{ formatDate(application.applied_at) }}</div>
      ...
      <p v-else class="mt-8 text-gray-500 text-center">No job applications found.</p>
    </div>
  </AppLayout>
</template>
